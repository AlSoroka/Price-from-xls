...
//Разбитваем название на куски не более 47 символов длиной
	...
	for (let i=0; i<8-base_price_length; i++) {base_price+=' '};
	base_price+='!';
	...
	for (let i=0; i<9-input_count_length; i++) {input_count+=' '};
	input_count+='!'	
    order+=input_count;  
	...
	for (let i=0; i<9-sum_disc_length; i++) {sum_disc+=' '};
	sum_disc+='!';		
	order+=sum_disc;  //скидка
	order+=' '+document.getElementById("sum_k_opl").innerHTML+'\n';  //сумма к оплате
