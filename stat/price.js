...
function sum_calculate(book_pref, book_price, book_count)
 {
	...
	var tr=document.getElementById(boko_pref);
	//alert(tr);

	if(!book_count.match('[1-9]+')) {
		{alert('Введите нужное Вам количество книг (целое число)');
		document.getElementById(id_count).focus();
		return}

		count= parseInt(book_count);
		// Сохраняем выбранные в LocalStorage
		localStorage[book_pref] = count; //установка значения
	...
	if ((count>9)&&(count<=49)) {
		sum_contract=base_sum*0.85;
	disc=Math.round(base_sum-sum_contract);
		//disc_str="<br> <span "+id_disc+" style='color:red;'> <b>Cкидка</b> (15%): "+disc.toFixed(2)+" руб. </span>";
		}
	else if ((count>49)&&(count<=99)) {
		sum_contract=base_sum*0.75;
	disc=Math.round(base_sum-sum_contract);
		//disc_str="<br> <span id='disc' style='color:red;'> <b>Cкидка</b> (25%): "+disc.toFixed(2)+" руб.</span>";
		}
	else if (count>99){
		sum_contract=base_sum*0.65;
		disc=Math.round(base_sum-sum_contract);
	...
	if (disc>0) {
		...
		document.getElementById(id_disc).innerHTML = disc.toFixed(2);
		...
		document.getElementById(id_disc).innerHTML = "&mdash;";

		}

	//alert (id_sum+'  '+sum_contract.toFixed(2))	;
...
// Этот массив назхывается idList. Итерируем по нему и получаем суммарные скидки, количества, значения.
	...
	if (total_count>0)	{
		...
		document.getElementById("sum_contract").style.visibility="visible" ;
...
function viewOrder()
	{
		...
		//var mainNews=document.getElementById('DontBelieve');
		//mainNews.style.visibility = (isClick==-1) ? "hidden" : "visible";
	...
	var tds, lineJSON;
		for (var i = 0; i <rows.length; i++)
		{
			...
			tds=rows[i].getElementsByTagName("td");
				order+='<id>'+tds[0].innerHTML + '</id> ';
				order+='<name>'+tds[1].innerHTML + '</name> ';
				order+='<price>'+tds[2].innerHTML+'</price> ';
				order+='<count>'+tds[3].firstElementChild.value+'</count> ';
				order+='<sum>'+tds[6].firstElementChild.innerHTML+'</sum>\n';
				//line={};   //Иначе в order попадала только последня строка
				order=order.replace('&nbsp;', ' ');
...
var focusHandler = function(event) {
	for (let id of idList) {
	 if (localStorage[id]>0 & localStorage[id] !== nul)
	 {tr.classList.add("selected");}
